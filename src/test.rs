fn main() {
    let mut zip = simzip::ZipInfo::new_with_comment("test.zip".into(), 
    "Generated by WebFolder (Java nio) 1.9.153, Copyright (c) 2009, 2025 by Dmitriy Rogatkin".into());
    let mut ign_entry = simzip::ZipEntry::new(".gitignore".into(), r#"src/test
test.zip"#.as_bytes().to_vec());
    #[cfg(feature = "deflate")]
    {ign_entry.compression = simzip::Compression::Deflate;}
    zip.add(ign_entry);
    let _ = zip.add(simzip::ZipEntry::new("test entry.txt".into(), "test content".as_bytes().to_vec()));
    zip.add(simzip::ZipEntry::from_file(&"README.md".to_string(), Some(&"doc".to_string())));
    #[cfg(feature = "test_unix")]
    {zip.add(simzip::ZipEntry::from_file(&"run script.sh".to_string(), Some(&"test".to_string())));
    zip.add(simzip::ZipEntry::from_file(&"read only.txt".to_string(), Some(&"test".to_string())));}
    match zip.store() {
        Ok(()) => println!{"Zipped good!"},
        Err(msg) => println!{"zip error: {msg}"}
    }
    
}